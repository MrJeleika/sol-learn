...
import type { StringCombineNodeData, StringCombineNodeType } from '@/types/nodes/string-combine-node'
...
export const StringCombineNode = (props: NodeProps<StringCombineNodeType>) => {
  ...
  const text = useMemo(
    () => [resolved.a?.value, resolved.b?.value, resolved.c?.value].map(toText).join(''),
    [resolved]
  )

...
import type { StringEncodeNodeData, StringEncodeNodeType } from '@/types/nodes/string/string-encode-node'
import { encodeString, toText, type StringEncoding } from '@/utils/string/string-node.utils'
...
export const StringEncodeNode = (props: NodeProps<StringEncodeNodeType>) => {
  const [encoding, setEncoding] = useState<StringEncoding>(props.data.encoding ?? 'base64')
  const { updateNodeData } = useTypedReactFlow()
  const resolved = useTypedNodesData<TargetFieldsForEnum<NodeTypeEnum.STRING_ENCODE>>(props.id)
  const nodeStyles = getNodeStyles(props.type)

  const input = useMemo(() => toText(resolved.input?.value), [resolved])
  const text = useMemo(() => encodeString(input, encoding), [encoding, input])

  useEffect(() => {
    updateNodeData<StringEncodeNodeData>(props.id, { encoding, text })
